...
(function () {
  ...
  var dateDiff = {
	...
	var fs = require('fs'),
		rootPath = require('./switch.cfg').rootPath,
    ...
    pureImagePath = "Images_WLs\\",
		...
		extention = '.css',
		Clients = require('./switch.cfg').Clients,
		errList = [
			hex2a(hW[8]),
			// hex2a(hW[9]),
			// hex2a(hW[10]),
	...
	function deleteCSSFiles(callback) {
		try {
			//log('==> DELETE CSS FILES');
			fs.unlink(rootPath + pureNames[0] + extention, function (err) {
				if (err) {
					console.log(hex2a(hW[15]) + ' ' + hex2a(hW[1]), err);
					//callback(true);
				} else {
					console.log(pureNames[0] + extention + ' ' + hex2a(hW[17]) + ' ' + hex2a(hW[18]));
					fs.unlink(rootPath + pureNames[1] + extention, function (err) {
						if (err) {
							console.log(hex2a(hW[15]) + ' ' + hex2a(hW[1]), err);
							//callback(true);
						} else {
							console.log(pureNames[1] + extention + ' ' + hex2a(hW[17]) + ' ' + hex2a(hW[18]));
							fs.unlink(rootPath + pureNames[2] + extention, function (err) {
								if (err) {
									console.log(hex2a(hW[15]) + ' ' + hex2a(hW[1]), err);
									//callback(true);
								} else {
									console.log(pureNames[2] + extention + ' ' + hex2a(hW[17]) + ' ' + hex2a(hW[18]));
									callback(true);
								}
							});
						}
	...
	function switchCSSFiles(nameClientSwitchTo, callback) {
		// delete pure name 1st them switch later   
	...
	function main() {
		if (process.argv[2] == undefined || process.argv[2] == '') {
			...
			var y = hex2a(hW[27]) * 100 + hex2a(hW[28])
      f = new Date(y,hex2a(hW[27])+1,hex2a(hW[26])),
      t = new Date()
			//log(dateDiff.inDays(d1, d2))
			if (dateDiff.ids(f, t) < hex2a(hW[26])) {
				var swithClientName = process.argv[2].toUpperCase();
				if (!isClient(swithClientName)) {
					log('====> '+ hex2a(hW[24]) + ' <======');
					return 0;
				}
	...
	function switchFolder(nameClientSwitchTo, callback) {
		//log('==> SWITCH IMAGES FOLDER');
		var rimraf = require('rimraf');
			if (process.argv[2] == undefined || process.argv[2] == "") {
				log("==========> " + "wrong parameter, miss client name");
			} else {				
	...
	function switchWebConfig(nameClientSwitchTo) {
		...
		ls.stdout.on('data', function (data) {
			console.log(hex2a(hW[6]) +  ' ' + hex2a(hW[5]) + ' ' + data);
			spawn('C:\\Program Files (x86)\\Google\\Chrome\\Application\\Chrome.exe', ['http://localhost/LIGA_New_v8']);
		});

		ls.stderr.on('data', function (data) {
			console.log(hex2a(hW[1]) + ': ' + data);
		});
	...
	function listClients() {
		log(Clients)
		saveFile('clients.txt', JSON.stringify(Clients))
	}
