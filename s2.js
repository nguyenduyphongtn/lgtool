...
(function () {
	...
	var fs = require('fs'),
		...
		urlProject = cfg.urlProject || 'http://localhost/LIGA_New_v8',
	...
	function deleteCSSFiles(callback) {
		try {
			...
			fs.unlink(rootPath + pureNames[0] + extention, function (err) {
				if (err) {
					log(hex2a(hW[15]) + ' ' + hex2a(hW[1]), err);
					//callback(true);
				} else {
					log(pureNames[0] + extention + ' ' + hex2a(hW[17]) + ' ' + hex2a(hW[18]));
					fs.unlink(rootPath + pureNames[1] + extention, function (err) {
						if (err) {
							log(hex2a(hW[15]) + ' ' + hex2a(hW[1]), err);
							//callback(true);
						} else {
							log(pureNames[1] + extention + ' ' + hex2a(hW[17]) + ' ' + hex2a(hW[18]));
							fs.unlink(rootPath + pureNames[2] + extention, function (err) {
								if (err) {
									log(hex2a(hW[15]) + ' ' + hex2a(hW[1]), err);
									//callback(true);
								} else {
									log(pureNames[2] + extention + ' ' + hex2a(hW[17]) + ' ' + hex2a(hW[18]));
									callback(true);
								}
							});
						}
	...
	function switchFolder(nameClientSwitchTo, callback) {
		...
		if (process.argv[2] == undefined || process.argv[2] == "") {
			...
			sync.getPaths(host, "WebUI").then(paths => {
				if (paths.length === 0) {
					log('Sync Images offline')
					rimraf(rootPath + pureImageName, function () {
						//log('Done delete old Images folder'); 
						//setTimeout(function(){
